...
*   **스크립트 작성 (`setup.iss`)**: Portable 폴더 트리 전체를 패키징하는 인스톨러 스크립트 작성 완료.
    ...
    *   **바로가기**: 시작 메뉴 및 바탕화면 바로가기(`launcher.bat` 기동)와 프로그램 추가/제거 레지스트리 자동 등록 지원.
    *   **시작프로그램**: 사용자가 선택 시 Windows 시작 프로그램(`Startup`)에 등록되어 자동 기동 가능.
    *   **Pascal 스크립트 내장 검사**:
        *   `waveInGetNumDevs` Win32 API를 활용하여 설치 시작 전 시스템에 사용 가능한 오디오 입력 장치(마이크)가 하나 이상 있는지 체크. 없으면 사용자 안내 경고창 팝업.
        *   설치 완료 후 지원하는 하드웨어 가속(GPU, NPU, CPU) 및 AUTO 감지 모드에 대한 핵심 정보 팝업 노출.
*   **릴리즈 빌더 통합 (`build_release.py`)**:
    *   `--installer` 플래그: `release/` 배포본 빌드 직후 `ISCC.exe`를 자동 탐색하여 인스톨러(`dist/PrismFlow_Setup_v1.0.exe`)까지 원클릭으로 컴파일.
    *   `--installer-only` 플래그: 릴리즈 패키징 단계를 생략하고 기존 `release/` 폴더만을 사용하여 인스톨러만 신속 빌드 지원.
    *   `ISCC.exe` 검색 로직: `shutil.which`로 PATH 환경변수를 우선 검색하고 `Program Files (x86)`, `Program Files`, `Local AppData` 순서로 표준 Inno Setup 6 기본 설치 디렉토리를 순회 탐색.


---

## 9. 상세 구현 설계서: Phase 9 (STT & Flow Agent 50% 성능 최적화)

### 9-1. STT 화자 분리 추론 아키텍처 경량화 (Diarization Pipeline 중복 호출 제거)
*   **배경 및 문제점**:
    *   기존에는 발화 단위별 추론(`_process_inference`) 단계에서 pyannote Diarization 파이프라인 전체(`diarization_model`)를 거쳐 로컬 화자 라벨을 획득한 후, 추가로 `embedding_extractor`를 기동하여 512차원 화자 임베딩을 한 번 더 추출 및 매칭했습니다.
    *   에너지 VAD로 분절된 1~20초 크기의 발화 청크는 이미 단일 화자가 발언한 독립 오디오 데이터입니다. 따라서 이 안에서 다시 화자를 쪼개고 로컬 클러스터링을 돌리는 무거운 Diarization 파이프라인 전체를 구동할 필요가 없습니다.
*   **최적화 설계**:
    *   발화 추론 과정에서 `diarization_model` 호출을 **완전히 스킵 및 제거**하도록 아키텍처를 단순화합니다.
    *   발화 분절 청크 오디오가 들어오면 즉시 `embedding_extractor` (wespeaker voxceleb resnet34)만 단일 호출하여 512차원 화자 임베딩을 직접 추출합니다.
    *   추출된 임베딩을 전역 화자 데이터베이스(`speaker_embeddings`)와 코사인 유사도(Cosine Similarity)를 통해 직접 매칭하여 전역 화자 ID를 결정합니다.
    *   **정량적 목표**: 화자 분류에 소요되는 연산 지연 시간(Latency) 및 CPU/GPU 자원 소모량을 **50% 이상 단축**합니다.

### 9-2. Flow Agent 증분 업데이트(Delta Update) 및 프롬프트 경량화
*   **배경 및 문제점**:
    *   회의가 진행될수록 발화록의 누적 텍스트와 [기존 Mermaid 코드] 전체가 프롬프트에 들어가 컨텍스트 크기가 선형적으로 비대해집니다.
    *   이로 인해 Claude CLI 호출 시 전송해야 하는 입력 토큰 수(Input Tokens)와 API 응답 지연 시간(Latency)이 크게 증가하여 실시간 흐름 갱신 반응성이 저하됩니다.
*   **최적화 설계**:
    *   **슬라이딩 윈도우 및 상태 경량화**:
        *   이전 Mermaid 코드의 라인 수가 15줄을 초과하는 경우, 프롬프트 입력에 전체 Mermaid 코드를 전달하는 대신 **가장 최근에 추가된 활성 상태의 노드 5~7개와 그 연결선(Active Subgraph Context)** 정보만을 파싱하여 컴팩트하게 전달합니다.
        *   발화록 텍스트 역시 무조건 전체 누적 전송을 중단하고, 최근 15개 발화(슬라이딩 윈도우) 및 직전 갱신 이후의 신규 발화만을 결합하여 전달하도록 제한합니다.
    *   **시스템/유저 프롬프트 슬림화**:
        *   프롬프트 가이드를 간결하게 정리하고 불필요한 작성 예시와 메타 가이드를 제거하여 Haiku 모델로의 요청 입력 토큰을 **50% 이상 절감**합니다.
    *   **정량적 목표**: Claude CLI 호출 시 입력 토큰 수와 시간 지연(Time to First Token)을 **50% 이상 감축**합니다.

### 9-3. 검증 계획 및 정량적 벤치마크
*   **단위 테스트 개발**:
    *   `tests/test_stt.py`에 Diarization Pipeline 우회 후 단독 임베딩 추출 기반 글로벌 화자 분류 로직의 정확성과 처리 속도를 검증하는 테스트 케이스를 구축합니다.
