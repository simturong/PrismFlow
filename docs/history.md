...
| **Phase 15: 상위 모델 배선 및 셋업** | ✅ 완료 | 2026-06-21 | medium/large-v3 Whisper 모델 셋업 스크립트 작성 및 미설치 에러 안내 UX 보강 |
| **Phase 16: 통합 회의 콘솔 & STT 튜닝** | ✅ 완료 | 2026-06-21 | Flow/Chat 콘솔 단일화, 채팅 토글, 자막 헤드라인 확대 및 핵심어 강조, STT 벤치마크 기반 튜닝 |
| **Phase 17: 회의 제어 & Mermaid 개편** | ✅ 완료 | 2026-06-21 | ⏹+▶/⏸ 2버튼 상태기계, Mermaid 라벨 잘림 해소 및 자유 재구성, STT medium 확정 및 interim 망각 수정 |
| **Phase 18: Mermaid 줌/팬 & 작업표시줄 상주** | ✅ 완료 | 2026-06-22 | svg-pan-zoom 및 Glassmorphic 줌 툴바, 줌 상태 보존, 작업표시줄 노출, 닫기 시 트레이 숨김, 심볼릭 아이콘 통합 |
| **Phase 19: Mermaid 박스 최적화 & STT 7초 분절** | ✅ 완료 | 2026-06-22 | Mermaid wrappingWidth 상향 및 박스 360px 강제 확장, STT VAD 7초 강제 분절로 왜곡 차단, 줌 툴바 3버튼 SVG 룩앤필 통합 |
...
## 🚀 Phase 17: 회의 제어 UX 재설계 · Mermaid 사용성/자유도 · 실시간 전사 개선 (2026-06-21)

### 1. 배경
Phase 16 사용자 E2E 후속 피드백. 단일 콘솔/헤드라인/CLI 분류/엔진모드는 정상이었으나 ①회의 제어 버튼이 직관과 다름 ②Mermaid 글자 잘림·세로 나열·좌우 여백 과다·subgraph 미출현 ③실시간 전사 지연 + interim이 앞 문장 잃음 ④채팅 토글 핸들 거슬림. 모델은 사용자 지시로 **medium 확정 후 빡세게 튜닝**.

### 2. 주요 구현 내용
- **회의 제어 상태기계(17-1)**: 기존 3버튼(시작/일시정지/정지)을 **⏹ 정지(좌) + ▶/⏸ 재생-일시정지 토글(우)** 2버튼으로 재설계. `_refresh_meeting_controls`가 (비활성→▶시작)/(진행→⏸)/(일시중지→▶재개)/(정지→비활성) 상태를 한 곳에서 동기화. `overlay.py`는 setText에 실제 글리프 문자가 박혀 있어 부분 편집이 위험했으므로 **전체를 \uXXXX escape 상수로 통일해 재작성**(편집 안전성 확보).
- **채팅 토글 이동(17-4)**: 우측 중앙의 세로 핸들을 제거(접힘 시 빈 여백 컬럼 사라짐)하고, 상단 컨트롤바 녹음표시 옆에 💬 토글 버튼을 두었다(`enable_chat_toggle`/`_on_chat_toggle_clicked`가 호스트의 `toggle_chat_panel`에 위임).
- **Mermaid 사용성·자유도(17-2)**: 폰트 28→20px, htmlLabels 라벨 줄바꿈(`white-space:normal`)·패딩↑로 한국어 잘림 해소. svg를 폭 100%로 채우고 세로는 자연 크기+컨테이너 스크롤로 두어 좌우 여백/축소를 완화. Flow 프롬프트를 append-Upsert 강제에서 **"사용성 우선 자유 재구성"**으로 전환 — subgraph 그룹핑 의무화, stale 노드 제거 허용, 12~15노드로 압축, 방향(TD/LR) 자유.
- **STT 실시간 개선(17-3)**: 기본 medium 확정(DB). endpoint 1.0→0.7초로 문장 단위 확정을 신속히 해 "확정 문장 누적"을 촉진. Phase 16-3에서 도입한 interim 4초 윈도우가 긴 발화의 앞부분을 잃게 한 부작용을 보정 — interim을 진행 발화 '전체'(상한 10초)로 되돌렸다(짧아진 발화 덕에 비용도 가벼움). `collapse_repetitions`(반복 환각 억제)는 유지.

### 3. 시행착오와 의사결정
- **medium 채택은 사용자 결정**: Phase 16-3 실측은 small을 권했으나, 사용자는 medium의 정확도 잠재력을 살리는 쪽을 택했다. 그래서 medium의 약점(확정 지연·반복 환각)을 구조 개선(endpoint 단축·interim 전체·반복 붕괴)으로 공격적으로 보정하는 방향으로 정리했다. (config 기본값은 신규 클론 안전을 위해 small 유지, 활성 기본은 DB로 medium.)
- **interim의 두 얼굴**: 16-3에서 "비용 상수화"를 위해 4초로 잘랐더니 17에서 "앞 문장 망각"이라는 사용성 문제가 드러났다. 두 요구가 상충(비용↓ vs 맥락 보존)했는데, **endpoint를 줄여 발화 자체를 짧게** 만들면 둘 다 해결됨을 깨달았다 — 발화가 짧으면 전체 interim도 가볍고 앞부분도 안 잃는다. 파라미터 하나(endpoint)가 두 문제의 공통 해법이었다.
- **글리프 vs escape**: 한글/아이콘 글리프가 소스에 박히면 부분 편집(Edit 매칭)이 깨진다. 컨트롤바처럼 글리프가 많은 위젯은 처음부터 \uXXXX escape로 두는 편이 유지보수에 안전함을 학습.

### 4. 교훈
- **상충하는 두 요구의 공통 상류 원인을 찾아라**: 비용·맥락보존이라는 상충을 파라미터 추가가 아니라 상류(발화 길이=endpoint)에서 동시에 해소했다.
- **사용성 우선 자유도**: Mermaid를 "정확한 누적 기록"이 아니라 "한눈에 읽히는 요약 지도"로 재정의(자유 재구성·stale 제거)하니, 프롬프트만으로도 가독성이 크게 개선될 여지가 생겼다(실 검증은 사용자 E2E로 확정).


---

## 🚀 Phase 18: Mermaid 흐름도 실시간 줌/팬, 작업 표시줄 활성화, 백그라운드 숨김 및 심볼릭 아이콘 통합 (2026-06-22)

### 1. 배경
실시간 회의 발화 증가에 따라 Mermaid 다이어그램(Flow)의 규모가 커지면서 노드가 축소되거나 일부 영역을 보기 힘든 가독성 문제가 발생했습니다. 이를 조작하기 위해 마우스 휠 줌(Zoom) & 좌클릭 드래그 팬(Pan)을 벤치마킹하여 도입했습니다. 동시에 메인 콘솔이 활성화될 때 작업 표시줄(Taskbar)에 뜨게 하고, 창 닫기 시 소멸하는 대신 작업표시줄에서 숨겨지고 트레이 상주(백그라운드)하도록 동작 구조를 변경했습니다. 또한 기존 컴퓨터/재생 아이콘을 PrismFlow 전용 프리미엄 심볼릭 아이콘으로 교체하고 회의 중 상태를 동적으로 표현하도록 통합했습니다.

### 2. 주요 구현 내용
- **오프라인 줌/팬 캔버스 (18-1, 18-4)**: 초경량 `svg-pan-zoom.min.js` 라이브러리를 로컬 resources 폴더에 번들하여 오프라인 배포성을 유지했습니다. 기존 `#diagram-container svg`의 인라인 크기 강제 스타일(`width: 100% !important;` 등)이 라이브러리의 DOM 좌표 변환과 부딪쳐 화면이 찌그러지던 문제를 CSS 리팩토링과 `overflow: hidden` 적용으로 해소했습니다.
- **Glassmorphic 줌 툴바 UI 및 API 연동 (18-2)**: 웹뷰 내 우측 하단에 absolute로 배치한 반투명 유리 컨트롤 바(➕, ➖, 🎯 fit/center, 🔄 reset)를 입히고, svg-pan-zoom API를 호출하도록 자바스크립트를 배선했습니다.
- **줌/팬 상태 보존(State Preservation) (18-3)**: 다이어그램이 8초/15초마다 새로 갱신될 때 초점이 1:1로 리셋되어 길을 잃던 치명적 문제를 해결하기 위해, 갱신 직전 줌/팬 값을 변수에 백업해두었다가 새 SVG 렌더 완료 직후 원래 위치로 복원하는 상태 유지 로직을 구현했습니다.
- **프리미엄 로고 이미지 생성 및 배치 (18-5)**: `generate_image`로 현대적인 네온 그라데이션 프리즘 로고(`app_icon.png`)를 디자인하여 resources 폴더에 이식했습니다.
- **작업 표시줄 노출 및 아이콘 지정 (18-6)**: `TranslucentOverlay` 창 플래그에서 작업 표시줄을 가두던 `Qt.Tool`을 제거하고 `Qt.Window | Qt.FramelessWindowHint`로 변경했으며, `setWindowIcon`을 통해 `app_icon.png`를 적용해 작업 표시줄과 창 이름에 매핑되도록 처리했습니다.
- **창 닫기 시 백그라운드 숨김 (18-7)**: `MeetingContext`에 `is_quitting` 필드를 신설하고, `closeEvent`에서 종료 신호가 아닌 일반 닫기일 때는 `event.ignore()` 및 `hide()` 처리하여 작업표시줄과 화면에서만 사라지도록 구현했습니다. 트레이 종료 메뉴(`exit_app`)에서만 `is_quitting = True`로 설정하고 `QApplication.quit()`를 불러 프로세스를 회수합니다.
- **트레이 아이콘 및 동적 상태 점 표시 (18-8)**: 트레이 기본 아이콘을 `app_icon.png`로 교체했으며, 회의 중일 때는 별도 파일 로드 없이 `QPainter`를 사용해 기본 아이콘 pixmap의 우측 하단 구석에 네온 레드 원(녹음 중 상태 점)을 동적으로 덧그려 `active_icon`을 생성하도록 구현하여 리소스 관리 효율을 높였습니다.
- **회귀 검증 (18-9)**: `test_flow.py` 리소스 검증 및 `test_ui.py` 다량 추가로 전체 101 passed, 1 skipped(실엔진 제외) 무결 테스트를 통과했습니다.

### 3. 시행착오와 의사결정
- **CSS 충돌 디버깅**: 외부 라이브러리(svg-pan-zoom)가 인라인 transform과 viewBox를 주도하기 때문에, 기존 HTML/CSS에 들어있던 `max-width`, `height: auto`와 같은 반응형 레이아웃 스타일이 있으면 스케일이 구겨지는 현상을 겪었습니다. CSS와 라이브러리의 역할 범위를 분리하여 CSS는 컨테이너의 영역 크기만 제공하고 SVG 스케일은 svg-pan-zoom에 일임하여 오동작을 말끔히 잡았습니다.
- **QPainter의 동적 표현**: 회의 시작 시 트레이 아이콘을 전환하기 위해 매번 하드코딩된 별도 이미지를 넣거나 파일이 누락되어 크래시가 나는 리스크 대신, 코드 기반의 동적 덧그리기를 설계해 불필요한 번들 자원 추가를 막고 미적 완성도를 끌어올렸습니다.
- **NameError 임포트 사전 차단**: `tray.py` 수정 시 `Qt.NoPen`을 사용할 때 `Qt` 임포트가 빠진 사실을 자율 교차 검증 단계에서 사전 포착하여, NameError 크래시를 컴파일 전 완벽 차단했습니다.

### 4. 교훈
- **라이브러리와 CSS 레이아웃은 공존하되 역할을 분리해야 한다**: DOM 변환을 가하는 외부 라이브러리를 입힐 때는 레이아웃의 고정 강제 속성들을 걷어내야 안정성을 보장할 수 있습니다.
- **동적 자원 생성의 강점**: 파이썬 코드로 그래픽 픽스맵을 조작하는 것은 정적 파일을 늘리는 것보다 유지보수와 배포 관점에서 훨씬 강력하고 세련된 접근법입니다.


---

## 🚀 Phase 19: Mermaid 박스 크기 최적화, 긴 발화 강제 분절(Force Commit) STT 튜닝 및 줌 툴바 UI 간소화 (2026-06-22)

### 1. 주요 구현 내용
- **Mermaid 박스 가로폭 확대 & 단어 줄바꿈 오동작 해결 (19-1)**: `mermaid_html.py`의 `wrappingWidth` 속성값을 500으로 상향하고, CSS 스타일에 `word-break: keep-all !important;`, `white-space: normal !important;`를 지정했습니다. 추가 E2E 피드백에 따라 박스 가로 폭을 고정 및 강제 확장할 수 있도록 `.nodeLabel` 요소에 `width: 360px !important;` 및 `display: block !important;` 스타일을 적용해 좁은 가로폭에서 글자가 세로로 심하게 부러지던 현상을 해소하고 넓고 가독성 높은 레이아웃을 완성했습니다.
- **STT 긴 발화 7초 강제 분절 (Force Commit) 배선 (19-2)**: `stt_agent.py` 내의 `max_utt_samples` 및 `interim_window_samples` 값을 기존 20.0초/10.0초에서 **7.0초**(`7.0 * sr`)로 단축했습니다. 사용자가 쉬지 않고 긴 이야기를 이어갈 때, VAD 엔드포인팅(0.7초)이 작동하지 않아 임시 전사 버퍼가 계속 늘어나고 Whisper가 앞 문장을 왜곡하거나 생략하는 문제를 차단합니다. 7초마다 강제로 현재까지의 발화를 `finalize()` 처리하여 데이터베이스에 영구 commit 및 자막창의 확정 영역으로 넘깁니다.
- **돋보기 줌 툴바 UI 및 룩앤필 정합 (19-3)**: 중복되던 리셋(🔄) 버튼을 제거하여 3개 버튼으로 정돈했습니다. ➕, ➖ 이모지와 네모 기호 간의 룩앤필 불일치를 해소하기 위해, 세 아이콘 모두 굵기 2.5px의 동일한 스타일을 가지는 인라인 라인 드로잉 SVG(확대/축소/사각형)로 전면 교체하여 시스템의 해상도나 폰트 기호 차이에 무관하게 고급스럽고 균일한 룩앤필을 갖추도록 정비했습니다.

### 2. 시행착오와 의사결정
- **Interim 단축의 두 얼굴과 강제 분절의 해법**: Phase 17에서 "앞문장 망각" 해결을 위해 interim 윈도우를 다시 전체(10초)로 늘렸지만, 이로 인해 긴 대화 시 Whisper가 앞 단어를 계속 왜곡하는 반복 수정 버그가 재발했습니다. 윈도우만 다시 줄이는 대신, **강제 분절 임계값을 7초로 단축**하여 Whisper가 처리할 최대 오디오 길이를 7초로 억제함과 동시에 7초 단위로 확정 전사(Commit) 처리를 내림으로써, interim 짤림도 없고 앞문장 왜곡도 발생하지 않는 최적의 균형점을 찾았습니다.

### 3. 교훈
- **사용자 피드백의 구체화**: "화면맞춤을 ㅁ 형태로 해달라"와 같은 사용자 사용성 피드백을 HTML Entity `&#9723;` 로 정교하고 브라우저 독립적으로 변환하면서 툴바를 간소화하여 미적 편의성과 심미적 premium look을 양립할 수 있었습니다.
