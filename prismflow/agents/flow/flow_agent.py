...
[기존 Mermaid 코드]는 직전 회의 지도이고, [최근 15개 회의 발화 내역]이 가장 최신 맥락입니다.
목표는 "지금 회의가 어떻게 흘러가는지 한눈에 보이는, 읽기 쉬운 지도"입니다. 누적 기록이 아니라 **사용성 우선의 요약 지도**를 만드세요.
...
[작성 규칙]
1. 출력 형식(엄수):
<현재 논의의 핵심 요약 1문장 (뉴스 자막용, 공백 포함 30자 내외)>
===
<flowchart TD 또는 flowchart LR로 시작하는 순수한 Mermaid 코드>
2. 마크다운 코드 펜스(```)나 안내 문구를 절대 넣지 말고 위 양식만 출력하세요.
3. **자유 재구성 허용**: 기존 구조에 무조건 덧붙이지 마세요. 더 명확하면 노드를 재배치·병합·이름변경해도 됩니다. **이미 종료됐거나 더는 중요하지 않은 화제의 노드는 과감히 제거**해 지도를 항상 간결하게 유지하세요.
4. **간결성**: 전체 노드는 최대 12~15개. 핵심 논점·결정·할 일 위주로 압축하고, 사소한 디테일은 생략하세요.
5. **그룹핑(중요)**: 서로 다른 대주제가 2개 이상이면 각 대주제를 `subgraph 제목 ... end` 로 묶으세요. 화면 좌우 공간을 활용하도록, 병렬적인 주제는 subgraph를 나란히 두거나 flowchart LR로 방향을 잡아도 됩니다(가독성 우선으로 방향·배치를 자유롭게 선택).
6. 노드 라벨은 짧게(가능하면 12자 이내, 길면 두 단어로). 인사말·잡담·중복 추임새는 노이즈로 제외하세요.
7. 화자 식별자(Speaker_00 등)는 절대 포함하지 마세요. 논의 흐름과 핵심 내용만으로 구성하세요.
"""
